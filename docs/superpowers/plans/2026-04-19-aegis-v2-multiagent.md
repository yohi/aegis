...
def test_review_agent_runtime_checkable() -> None:
    # @runtime_checkable 付きの Protocol は isinstance(...) が TypeError を出さない。
    # `runtime_checkable(ReviewAgent)` を直接 assert しても引数のクラスをそのまま返すため
    # 常に truthy になり、実行時検査可能性の回帰防止にならない。
    class Dummy:
        persona = AgentPersona.SENTINEL
        async def review(self, ctx):  # noqa: ANN001
            raise NotImplementedError

    assert isinstance(Dummy(), ReviewAgent)
...
**重要（Python 構文制約）:** `from __future__ import annotations` はモジュール先頭（docstring 直後）にしか置けないため末尾追記では絶対に含めないこと。既存 `src/core/protocols.py` には既に先頭で宣言済みであることを事前に `head -5 src/core/protocols.py` で確認してから作業する。不在だった場合は別コミットで先頭へ移送する（本 Task には含めない）。

```python
# ---------------------------------------------------------------------------
# v2 protocols (additions only; v1 protocols above remain unchanged)
# `from __future__ import annotations` は先頭で既に宣言済み（追記しないこと）。
# ---------------------------------------------------------------------------
from collections.abc import Sequence
...
class _FakeAnthropicClient:
    ...
    async def create(self, **kwargs):  # noqa: ANN003
        ...
        # probe 呼出しは output_config を含む。max_tokens の値ではなく
        # extra_body.output_config の有無で判定する（thinking 有効時に
        # max_tokens=1 を送るとトークン長検証で先にエラーになるため）。
        if self.probe_raises and "output_config" in (kwargs.get("extra_body") or {}):
...
class _FakeToolUseResponse:
    """JSON mode 応答: final_output ツール呼出しを擬似する."""
    content = [
        type("ToolUseBlock", (), {
            "type": "tool_use",
            "name": "final_output",
            "input": {"decision": "uphold", "rationale": "ok"},
        })()
    ]
...
@pytest.mark.asyncio
async def test_json_mode_extracts_tool_use_input() -> None:
    from src.v2.llm.providers.anthropic import AnthropicProvider
    import json

    class _ToolClient(_FakeAnthropicClient):
        async def create(self, **kwargs):  # noqa: ANN003
            self.calls.append(kwargs)
            if self.probe_raises and "output_config" in (kwargs.get("extra_body") or {}):
                raise _FakeBadRequestError("output_config rejected")
    ...
    p = await AnthropicProvider.create(
        model_id="claude-opus-4-7",
        client_factory=lambda: _ToolClient(probe_raises=False),
        bad_request_error=_FakeBadRequestError,
    )
    resp = await p.complete(
        system=[SystemBlock(text="SYS")],
    ...
    # tool_use.input が JSON 文字列として content に入っていること
    payload = json.loads(resp.content)
    assert payload == {"decision": "uphold", "rationale": "ok"}
...
class AnthropicProvider:
    ...
    async def create(
        ...
        *,
        ...
        try:
            # thinking={"type":"adaptive"} を有効にすると extended thinking の予算
            # が必要になるため max_tokens=1 では probe 自体がトークン長検証で失敗し、
            # output_config の受容可否を判別できない。最小でも 1024 を確保する。
    ...
    async def complete(
        ...
        *,
        ...
        return self._adapt_response(raw, response_format=response_format)
    ...
    def _adapt_response(
        ...
        *,
        ...
        # JSON mode は final_output ツールに強制しているため、応答は tool_use ブロック
        # として返る。その場合は tool.input を JSON 文字列にシリアライズして content に入れる。
        # text mode の場合は従来通り content の text ブロックを連結する。
        ...
        for b in raw.content:
            ...
            elif btype == "tool_use" and getattr(b, "name", None) == "final_output":
                json_payload = _json.dumps(
                    getattr(b, "input", {}) or {}, ensure_ascii=False,
                )
        if response_format == "json" and json_payload is not None:
            text = json_payload
        elif response_format == "json" and json_payload is None:
            # JSON mode を要求したのに tool_use が返らなかった異常ケース。
            # text 連結を最後のフォールバックとするが、呼出し側（AgentExecutor）で
            # JSON パースが失敗した時点で structlog.error("agent_parse_failed") を記録する。
...
**ReAct/tool-execution ループが不要な理由（設計上の明示）:**

Automation 経路ではエージェントが LLM 呼出し中に `InformationSource`（NotebookLM 等）を追加問い合わせする必要はない。設計書 §3.2 (1) と §6.5 の通り、`prepare_context` ノードが:

- `NotebookLMSource.query(...)` を **事前実行** し `NotebookContext` を構築
- 関連文書 / prior_form もすべて ingest 済みで `AgentContext` に格納

したがって各 `ReviewAgent.review(ctx)` は **単一ターン** の LLM 呼出しで完結する。`AgentExecutor` は意図的に tool-loop を持たず、これが Boundary-based Shielding（§1.2）の入力側 shield を 1 点で強制できる構造の基礎になっている。

Interactive 経路では Cursor Agent が native に tool-use ループを処理するため、MCP ツール（`notebook_query` 等）を介したインタラクティブ問い合わせは Cursor 側で完結する。Python バックエンドに tool-loop は実装しない（設計書 §1.1 / §7.1 参照）。
...
**実装方針:**
- DomainAgent: `ctx.notebook_context`（`prepare_context` で事前フェッチ済みの `SourceExcerpt` 束）を user_content に埋め込み、LLM 呼出しは 1 ターンで完結（Task 4.1 の「ReAct/tool-execution ループが不要な理由」を参照）。応答 finding には `authority_level=ABSOLUTE` を埋め込む
- E14 対応: 同一論点で 2 件以上の ABSOLUTE 抜粋が矛盾 → `FindingV2.comment_type=QUESTION`, `bug_phenomenon=DESIGN_EXPRESSION`。矛盾検出は LLM プロンプト側で指示（`prompts/domain.xml` 参照）
